...
if ($_SESSION['user']['user_type'] == 1) {
	$sql = "SELECT request_header.transpoType, transportation_type.type,request_header.requested_by,request_details.request_header_id,mst_admin_user.fname ,request_header.invoice_no, request_header.id , request_header.at_pickup,request_header.at_dropoff,mst_user.firstname, mst_user.lastname, request_details.item, package_type.package_name, request_details.sender_name, request_details.sender_contact, request_details.recepient_name, request_details.recepient_number, request_details.origin_lat, request_details.origin_lng, request_details.destination_lat, request_details.destination_lng , request_details.origin_address, request_details.destination_address, request_details.distance, request_details.cost, request_details.note, request_header.status, request_details.tod, request_details.origin_lat FROM request_details LEFT JOIN request_header ON request_header.id = request_details.request_header_id LEFT JOIN mst_user ON request_header.requested_by = mst_user.id  LEFT JOIN package_type ON request_details.package_type = package_type.id LEFT JOIN mst_admin_user ON request_header.assigned_driver = mst_admin_user.id LEFT JOIN transportation_type ON request_header.transpoType = transportation_type.id WHERE (request_header.status = 'CONFIRMED' OR request_header.status = 'ACCEPTED') AND (request_details.details_status = 'CONFIRMED' OR request_details.details_status = 'ACCEPTED') GROUP BY request_header.id ORDER BY request_header.id ASC";
}else{
$sql = "SELECT SUBSTRING_INDEX(request_details.origin_lat, '.', 1) as location_lat, SUBSTRING_INDEX(request_details.origin_lng, '.', 1) as location_lng, request_header.transpoType, transportation_type.type,request_header.requested_by,request_details.request_header_id,mst_admin_user.fname ,request_header.invoice_no, request_header.id , request_header.at_pickup,request_header.at_dropoff,mst_user.firstname, mst_user.lastname, request_details.item, package_type.package_name, request_details.sender_name, request_details.sender_contact, request_details.recepient_name, request_details.recepient_number, request_details.origin_lat, request_details.origin_lng, request_details.destination_lat, request_details.destination_lng , request_details.origin_address, request_details.destination_address, request_details.distance, request_details.cost, request_details.note, request_header.status, request_details.tod, request_details.origin_lat FROM request_details LEFT JOIN request_header ON request_header.id = request_details.request_header_id LEFT JOIN mst_user ON request_header.requested_by = mst_user.id  LEFT JOIN package_type ON request_details.package_type = package_type.id LEFT JOIN mst_admin_user ON request_header.assigned_driver = mst_admin_user.id LEFT JOIN transportation_type ON request_header.transpoType = transportation_type.id WHERE (request_header.status = 'CONFIRMED' OR request_header.status = 'ACCEPTED') AND (request_details.details_status = 'CONFIRMED' OR request_details.details_status = 'ACCEPTED') GROUP BY request_header.id HAVING SUBSTRING_INDEX(request_details.origin_lat, '.', 1) = $locationIndexLat AND SUBSTRING_INDEX(request_details.origin_lng, '.', 1) = $locationIndexLng ORDER BY request_header.id ASC";
}
$sql = $conn->query($sql);
...
while($row = mysqli_fetch_assoc($sql)){
	...
	$result[$count]['request_header_id'] = $row['request_header_id'];
	$result[$count]['at_pickup'] = $row['at_pickup'];
	$result[$count]['at_dropoff'] = $row['at_dropoff'];
	$result[$count]['invoice_no'] = $row['invoice_no'];
